...
export function AllOnXPage() {
  ...
  const itinerary = lang === 'de'
    ? ['Ankunft & 3D-Diagnostik', 'Implantat-OP / temporäre Zähne', 'Erholung & Tirana entdecken', 'Kontrolle & Anpassungen', 'Finale Übergabe / Heimreise']
    : ['Arrival & 3D diagnostics', 'Implant surgery / temporary teeth', 'Recovery & discover Tirana', 'Check-up & adjustments', 'Final handover / return trip'];

  return (
    <>
      <Hero
        title={lang === 'de' ? 'All-on-X Komplettpakete' : 'All-on-X Packages'}
        subtitle={lang === 'de' ? 'Feste Zähne, Hotelberatung, Transfer und klarer Reiseplan aus einer Hand.' : 'Fixed teeth, hotel guidance, transfers and a clear itinerary in one place.'}
        image={images.clinic}
      />
      ...
      <section className="py-section-padding bg-surface">
        <div className="max-w-[900px] mx-auto px-gutter">
          <h2 className="font-headline-md text-headline-md text-primary mb-8">
            {lang === 'de' ? '7-Tage Beispiel-Route' : 'Sample 7-Day Itinerary'}
          </h2>
...
export function StayPage() {
  ...
  return (
    <>
      <Hero
        title={lang === 'de' ? 'Aufenthalt & Logistik in Tirana' : 'Stay & Logistics in Tirana'}
        subtitle={lang === 'de' ? 'Direktflüge, Transfers, Hotels, Visa und Freizeit — stressfrei organisiert.' : 'Flights, transfers, hotels, visas and sightseeing — organized stress-free.'}
...
export function AftercarePage() {
  ...
  return (
    <>
      <Hero
        title={lang === 'de' ? 'Nachsorge, Garantie & Sicherheit' : 'Aftercare, Warranty & Safety'}
        subtitle={lang === 'de' ? 'Klare Pflegepläne, Remote-Follow-ups und Notfallkontakt auch nach Ihrer Rückkehr.' : 'Clear care plans, remote follow-ups and emergency contact after you return home.'}
      />
      <section className="py-section-padding bg-surface">
...
export function BlogPage() {
  const { lang } = useI18n();
  const posts = lang === 'de'
    ? ['Was kosten Veneers in Deutschland wirklich?', 'E-Max vs. Zirkonia: welches Material passt?', 'Dentalreise nach Tirana: Checkliste für deutsche Patienten']
    : ['What do veneers really cost in Germany?', 'E-Max vs. Zirconia: which material fits?', 'Dental trip to Tirana: checklist for German patients'];

  return (
    <>
      <Hero
        title={lang === 'de' ? 'Ratgeber für Veneers & Dentalreisen' : 'Veneer & Dental Tourism Guides'}
        subtitle={lang === 'de' ? 'Praktische Artikel zu Kosten, Materialien, Reiseplanung und Pflege.' : 'Practical articles about costs, materials, travel planning and care.'}
      />
      <section className="py-section-padding bg-surface-container-low">
        <div className="max-w-[1200px] mx-auto px-gutter grid md:grid-cols-3 gap-8">
          {posts.map((post, i) => (
            <article key={post} className="bg-white border border-outline-variant rounded-xl overflow-hidden">
              ...
              <div className="p-6">
                ...
                <Link to="/contact" className="text-primary font-bold">
                  {lang === 'de' ? 'Beratung dazu anfragen' : 'Ask about this'} →
                </Link>
              </div>
            </article>
          ))}
...
export function FacilityPage() {
  const { lang } = useI18n();
  const technologies = lang === 'de'
    ? ['3D Intraoral Scanner', 'CAD/CAM Fräsen', 'Digital Smile Design', 'Sterile OP-Zonen']
    : ['3D Intraoral Scanner', 'CAD/CAM Milling', 'Digital Smile Design', 'Sterile surgical zones'];

  return (
    <>
      <Hero
        title={lang === 'de' ? 'Klinik, Technologie & Sicherheit' : 'Clinic, Technology & Safety'}
        subtitle={lang === 'de' ? 'CAD/CAM, 3D-Scanner, digitale Smile-Planung und strenge Sterilisationsprotokolle.' : 'CAD/CAM, 3D scanners, digital smile planning and strict sterilization protocols.'}
        image={images.clinic}
      />
      <section className="py-section-padding bg-surface">
